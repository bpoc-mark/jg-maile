...
$(document).ready(function () {
  ...
  var $dashOffset = $(".swirl-1-pc").css("stroke-dashoffset"); //on a scroll event - execute function

  $(window).scroll(function () {
    //calculate how far down the page the user is 
    var $percentageComplete = $(window).scrollTop() / ($("html").height() - $(window).height()) * 300; //convert dashoffset pixel value to interger

    var $newUnit = parseInt($dashOffset, 15); //get the value to be subtracted from the 'stroke-dashoffset'

    var $offsetUnit = $percentageComplete * ($newUnit / 300); //set the new value of the dashoffset to create the drawing effect
